...
app.post('/api/v1/privileges', async (req, res) => {
  ...
  if (data.operation_type === 'FILL_IN_BALANCE') {
    ...
    hQuery =
    `insert into Privilege_history(id, privilege_id, ticket_uid, datetime, balance_diff, operation_type) values (${(await cnt).rows.length + 1}, ${priv_id}, '${ticket_uid}', now(), ${value * -1}, 'FILL_IN_BALANCE') returning ticket_uid;`;
  }
  const res_insert = privileges_db.query(hQuery)
  console.log((await res_insert))
